...
describe('AuthService', () => {
  ...
  it('registers a user and returns access and refresh tokens', async () => {
    ...
    jwtService.signAsync
      ...
      .mockResolvedValueOnce('jwt-refresh-token');
    ...
    expect(prismaService.user.update).toHaveBeenCalledWith({
      where: { id: 'user-1' },
      data: {
        refreshTokenHash: expect.stringMatching(/^[0-9a-f]+:[0-9a-f]+$/),
      },
    });
  ...
  it('refreshes tokens when refresh token is valid', async () => {
    const refreshSalt = '1234567890abcdef1234567890abcdef';
    const storedRefreshHash = scryptSync(
      'valid-refresh-token',
      refreshSalt,
      64,
    ).toString('hex');
    ...
    prismaService.user.findUnique.mockResolvedValue({
      ...
      email: 'john@example.com',
      password: 'stored-password',
      refreshTokenHash: `${refreshSalt}:${storedRefreshHash}`,
      createdAt: new Date('2026-04-20T00:00:00.000Z'),
      updatedAt: new Date('2026-04-20T00:00:00.000Z'),
    });
    prismaService.user.update.mockResolvedValue({});
  ...
  it('clears stored refresh token on logout', async () => {
    jwtService.verifyAsync.mockResolvedValue({
      sub: 'user-1',
      email: 'john@example.com',
      name: 'John',
